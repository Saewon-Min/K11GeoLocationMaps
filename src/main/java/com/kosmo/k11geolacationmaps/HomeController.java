package com.kosmo.k11geolacationmaps;
...
public class HomeController {
	...
	private static final String apiKey ="AIzaSyC_27HJSmnR1pr4xpIMEuI5tx_-4qDhgKE";
	...
	// 내 위치기반 시설물 반경 검색
	@RequestMapping(value="/GeoLocation/04SearchRadius.do",
			method=RequestMethod.GET)
	public String geoFunc4(Model model, HttpServletRequest req) {
		
		model.addAttribute("apiKey",apiKey);
		
		// 폼값 받기(String으로 전송된 값을 정수 혹은 실수형으로 변경함)
		...
		/* 
		전체 페이지 수( 검색결과 / 출력할개수 ) 
			- 페이지 번호를 대체할 <select> 태그에서 사용할 속성값
		*/
